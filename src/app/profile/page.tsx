    ...
    <div className="bg-zinc-950 px-4 py-8 sm:px-6 lg:px-8 max-w-2xl mx-auto w-full font-sans">
      ...
      <form onSubmit={handleSave} className="space-y-8">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-6 space-y-6">
          <h3 className="text-sm font-semibold text-zinc-200 border-b border-zinc-850 pb-3">Información Personal</h3>
          
          <div>
            <label className="block text-xs text-zinc-400">Email registrado</label>
            <div className="mt-2 text-sm text-zinc-300 font-mono select-all bg-zinc-900 border border-zinc-850 rounded-md px-3 py-2">
              {user?.email}
            </div>
          </div>

          <div>
            <label htmlFor="username" className="block text-xs text-zinc-400">Nombre de usuario</label>
            <div className="mt-2">
              <input
                id="username"
                type="text"
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-6 space-y-6">
          ...
          <div className="rounded-lg bg-zinc-950 border border-zinc-800/80 p-4 text-xs text-zinc-400 flex items-start gap-3">
          ...
          <div className="flex items-center justify-between bg-zinc-900 border border-zinc-850 rounded-lg p-4">
            <div>
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-hidden ${
                isPremium ? 'bg-zinc-50' : 'bg-zinc-800'
              }`}
            >
              <span
                className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-zinc-950 shadow-sm ring-0 transition duration-200 ease-in-out ${
                  isPremium ? 'translate-x-5 bg-zinc-950' : 'translate-x-0 bg-zinc-400'
                }`}
              />
            </button>
          </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-zinc-850">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border border-zinc-850 px-3 py-2 text-xs font-semibold text-red-400 hover:text-red-300 hover:bg-zinc-900 transition-colors"
          >
          ...
          <button
            type="submit"
            disabled={saving}
            className="rounded-md bg-zinc-50 px-4 py-2 text-xs font-semibold text-zinc-950 hover:bg-zinc-200 transition-all disabled:opacity-50"
          >
